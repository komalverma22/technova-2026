...
export default function EditEventModal({ event, onClose, onSuccess }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    minTeamSize: String(event.minTeamSize ?? 1),
    maxTeamSize: String(event.maxTeamSize ?? event.maxTeaSize ?? 4),
    date: toDatetimeLocal(event.date),
    venue: event.venue ?? "",
    rules: event.rules ?? "",
  });
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [imagePreview, setImagePreview] = useState<string>(event.imagePath ?? "");
